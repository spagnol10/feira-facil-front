...
export default function OrderView() {
  ...
  function handleVerifyPayment() {
    ...
    if (token && orderResponse) {
      handleGetSaleData(orderResponse.id, token).then((res) => {
    ...
    isAuth && (
      <>
        ...
        <DefaultScreenLayout screen={EnumScreen.ORDER}>
          <div className="flex flex-col md:h-full">
            ...
            <div className="w-full h-full flex gap-10 flex-col md:flex-row">
              <div className="w-full md:w-3/5 lg:1/2">
                <div>
                  <h2
                    title={orderResponse && orderResponse.id}
                    className="text-dark-color text-lg font-medium"
                  >
                    {orderResponse
                      ? "Pedido#" + orderResponse.id.substring(0, 6)
                      : "Cliente"}
                  </h2>
                        ...
                        <div className="flex flex-col gap-2 w-full justify-center mt-2">
                          ...
                          {ordertems.map((item) => (
                            ...
                            >
                              ...
                              <p className="w-full text-sm text-right text-secondary">
                                {formatMoneyWithSign(
                                  item.quantity * item.procuct.sellingPrice
                                )}
                              </p>
                            </div>
                          ))}
                      ...
                      <>
                        <span className="w-full lg:max-w-64">
                          <DefaultSelectOne
                            handleSelect={(val) =>
                              setPaymentType(
                                findEnumByValue(EnumPaymentType, val)
                              )
                            }
                            inputHeaderText="Forma de pagamento"
                            optionValue={paymentType ?? "Selecione"}
                            options={[
                              EnumPaymentType.CREDIT_CARD,
                              EnumPaymentType.DEBIT_CARD,
                            ]}
                          />
                        </span>
                        <span className="w-full lg:max-w-32">
                          <DefaultSelectOne
                            handleSelect={(val) => setInstallment(val)}
                      ...
                      <span className="flex gap-2 mt-6 items-center">
                        ...
                        <div
                          className={`w-3 h-3 rounded-full ${
                            isPaid ? "bg-primary" : "bg-yellow-400"
                          }`}
                        />
                      </span>
                    </div>
                  ...
                  {!orderResponse && (
                    <span className="xl:w-[402px] flex justify-end mt-2">
                      <span className="w-60 -mr-2.5">
                        <Button
                          icon={<PriceChangeIcon fill="#245F40" />}
                          text="Acréscimos e Descontos"
                          className="text-secondary underline"
            ...
            <div className="w-full h-10 flex mt-10 md:mt-auto justify-end gap-4">
              {orderResponse &&
                !isPaid &&
                orderResponse.status == "AGUARDANDO PAGAMENTO" && (
                  <span className="w-full md:w-48">
                    <Button
                      text={"Desistir pagamento"}
              ...
              {orderResponse && isPaid && (
                <span className="w-full md:w-32">
                  <Button
                    text={"Estornar"}
                    className="bg-red-color text-white"
                ...
                <span className="w-full md:w-40">
                  <Button
                    disabled={
                      ...
                      isNullOrEmpty(selectedProducts) ||
                      isNullOrEmpty(paymentType) ||
                      (equalsEnum(paymentType, EnumPaymentType.CREDIT_CARD) &&
                        isNullOrUndefined(installment))
                    }
              ...
              {orderResponse &&
                (isPaid ||
                  orderResponse.status == "PAGAMENTO ESTORNADO" ||
                  orderResponse.status == "DESISTÊNCIA PAGAMENTO") && (
                  <span className="w-full md:w-48">
                    <Button
                      text={"Novo Pedido"}
          ...
          {showFindUser && (
            <DefaultDialog
              dialogTitle="Selecionar cliente"
              className="h-3/5 md:h-fit min-h-[480px]"
              handleClose={() => setShowFindUser(false)}
            >
          ...
          {showRegisterUser && (
            <DefaultDialog
              dialogTitle="Cadastrar novo cliente"
              className="h-fit"
              handleClose={() => setShowRegisterUser(false)}
            >
              <div className="flex flex-col gap-6">
                ...
                <CustomInput
                  ...
                  invalid={
                    newUser?.phone ? !validatePhone(newUser?.phone) : false
                  }
                ...
                <span className="mt-6">
                  <Button
                    ...
                    text="Cadastrar"
                    disabled={
                      isNullOrEmpty(newUser?.name) ||
                      isNullOrEmpty(newUser?.document)
                    }
                  />
                </span>
              </div>
          ...
          {showProducts && (
            <DefaultDialog
              ...
              className="h-fit md:w-[60%] lg:w-[80%] xl:w-[60%]"
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="h-[400px] overflow-y-auto flex flex-row flex-wrap gap-4">
                  {filteredProducts.map((item) => (
                    <div
                      key={item.procuct.id}
                      className="flex gap-4 w-full h-36 lg:w-[48%] p-6 bg-white rounded-2xl shadow-lg"
                    >
                      <div className="flex w-full justify-between">
                        <span className="flex flex-col">
                          <span className="w-20">
                            <Image
                              src={item.procuct.imageUrl ?? ""}
                              alt={item.procuct.name}
                              width={58}
                              height={40}
                            />
                          </span>
                        ...
                        <span className="w-full flex flex-col gap-4 items-center">
                          <p className="text-base text-secondary">Valor</p>
                        ...
                        <div className="flex flex-col gap-2">
                          <button
                            className="w-full h-12 bg-primary rounded-md"
                            onClick={() =>
                              item.quantity + 1 <= item.procuct.stock &&
                          ...
                          <button
                            className="w-full h-12 bg-red-300 rounded-md"
                            onClick={() =>
                              item.quantity > 0 &&
                              updateProductQuantity(
                                item.procuct.id,
          ...
          {showAdditionDiscount && (
            <DefaultDialog
              dialogTitle="Acréscimos e descontos"
              className="h-fit"
              handleClose={() => setShowAdditionDiscount(false)}
            >
              <div className="flex flex-col gap-6">
                <CustomInput
                  id="addition"
                  value={formatCurrency(increaseDiscount.addition, true)}
                  inputHeaderText="Valor acréscimo"
                  onChange={(e) => handleCurrencyChange(e, setIncreaseDiscount)}
                />
                <CustomInput
                  id="discount"
                  value={formatCurrency(increaseDiscount.discount, true)}
                  inputHeaderText="Valor desconto"
                  onChange={(e) => handleCurrencyChange(e, setIncreaseDiscount)}
                />
                <span className="mt-6">
                  <Button
                    onClick={() => setShowAdditionDiscount(false)}
                    text="Adicionar"
                  />
                </span>
              </div>
          ...
          {showWaitingPayment && (
            <DefaultDialog
              dialogTitle="Aguardando pagamento"
              className="h-fit"
              handleClose={() => setShowWaitingPayment(false)}
            >
              <div className="flex flex-col gap-4">
                ...
                <p className="w-5/6 text-center text-sm text-secondary self-center">
                  Realize o processo de pagamento na maquininha, após isso seu
