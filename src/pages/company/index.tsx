...
import {
  handleCreateAcquirer,
  handleCreateCompany,
  handleFetchAcquirerCompanyData,
  handleFetchCompanyData,
} from "../../repositories/company.repository";
import {
  fecthExternalCitiesByState,
  handleFetchExternalCnpjData,
...
import {
  ...
  getAccountType,
  getToken,
  getTransferIntervalByString,
  getTransferIntervalDescription,
  getWeekDayByString,
...
export default function CompanyView() {
  ...
  const [registerStep, setRegisterStep] = useState(
    EnumRegisterCompanyStep.REGISTER_COMPANY
  );
  const [stepsTaken, setStepsTaken] = useState<Array<EnumRegisterCompanyStep>>([
    EnumRegisterCompanyStep.REGISTER_COMPANY,
  ]);
  const [selectedPlan, setSelectedPlan] = useState<EnumPlanType>();
  const [fairs, setFairs] = useState<Array<Fair>>();
  const [fairsFilter, setFairsFilter] = useState({
    state: "",
    city: "",
    citiesByState: [""],
  });
  ...
  const [loadingText, setLoadingText] = useState<
    EnumLoadingTextType | undefined
  >(EnumLoadingTextType.DEFAULT_LOADING_TEXT);
  ...
  useEffect(() => {
    if (equalsEnum(screenProcess, EnumCompanyScreen.BANK_ACCOUNT)) {
      if (!company?.hasAcquirerConfig) {
        let document = isIndividualCompany()
          ? formatCPFInput(owner?.document!)
          : formatCNPJInput(company?.document!);

        setAcquirerConfig(
          AcquirerConfig.defaultWithDocumentBank(document!, bankCodes[0].nome)
        );

        return;
      }

      if (company?.id && acquirerConfig === undefined) {
        let token = getToken();
  ...
  function handleListFairs(city?: string, state?: string) {
    ...
    if (token) {
      fecthFairsBy(state ?? fairsFilter.state, city ?? fairsFilter.city, token)
        .then(setFairs)
        .catch(() =>
          handleErrorToast("Erro ao buscar feiras, tente novamente.")
        );
    }
  }
  ...
  function handleStartCreateFair() {
    setRegisterFair(
      new Fair({
        address: new Address({
          state: fairsFilter?.state,
          city: fairsFilter?.city,
        }),
      })
    );
  }
  ...
  function handleUpdateView(step: EnumRegisterCompanyStep) {
    ...
    switch (step) {
      ...
      case EnumRegisterCompanyStep.SELECT_PLAN:
        newArray.push(
          EnumRegisterCompanyStep.REGISTER_OWNER,
          EnumRegisterCompanyStep.FAIRS,
          step
        );
        setStepsTaken(newArray);
        setRegisterStep(step);
  ...
  function validateDisable(step?: EnumRegisterCompanyStep) {
    switch (step ?? registerStep) {
      ...
      case EnumRegisterCompanyStep.FAIRS:
        return registerFair
          ? !isValidFairData()
          : !(company?.fairs && company?.fairs?.length! > 0);
      case EnumRegisterCompanyStep.SELECT_PLAN:
        return !selectedPlan;
      default:
        break;
  ...
  function isValidCompanyData() {
    return (
      (isValidObjectRequiredFields(company, ["imageUrl", "fairs"]) &&
        isValidObjectRequiredFields(company?.address, [
          "number",
          "complement",
        ]) &&
  ...
  function isValidOwnerData() {
    return (
      isValidObjectRequiredFields(owner) &&
      isValidObjectRequiredFields(owner?.address, [
        "number",
        "complement",
        "id",
      ]) &&
  ...
  function isValidFairData() {
    return (
      isValidObjectRequiredFields(registerFair, ["id"]) &&
      isValidObjectRequiredFields(registerFair?.address, [
        "number",
        "complement",
      ]) &&
  ...
  function validFairHour(hour: string) {
    ...
    if (
      registerFair?.startAt &&
      registerFair?.endAt &&
      isValidHour(registerFair.startAt) &&
      isValidHour(registerFair.endAt)
    ) {
      return (
        parseInt(onlyNumbers(registerFair.endAt)) >
        parseInt(onlyNumbers(registerFair?.startAt))
      );
    }
    ...
    if (token && acquirerConfig) {
      let payload = {
        ...
        bankAccount: {
          ...
          holderDocument: onlyNumbers(
            acquirerConfig.bankAccount!.holderDocument!
          ),
          bank: bankCodes.find(
            (bank) => bank.nome === acquirerConfig.bankAccount!.bank
          )?.codigo,
        },
      } as AcquirerConfig;
  ...
  function defineAccountType(val: string) {
    let accType = equalsStr(val, "Conta Corrente")
      ? EnumAccountTypeType.CHECKING
      : EnumAccountTypeType.SAVINGS;
    ...
    setAcquirerConfig((prevState: any) => {
      return {
        ...
        ["transferConfig"]: {
          ...
          transferDay: equalsEnum(
            enumTransferInterval,
            EnumTransferIntervalType.WEEKLY
          )
            ? getWeekDayByString(val)
            : undefined,
        },
      };
    });
  ...
  function isValidBankAccountData() {
    return (
      acquirerConfig?.bankAccount?.branchNumber &&
      ...
      ((acquirerConfig?.transferConfig?.enabled &&
        (equalsEnum(
          acquirerConfig?.transferConfig?.transferInterval,
          EnumTransferIntervalType.DAILY
        ) ||
          (acquirerConfig?.transferConfig?.transferDay &&
            acquirerConfig?.transferConfig?.transferInterval))) ||
        !acquirerConfig.transferConfig?.enabled)
    );
  }
              ...
              <>
                ...
                <div className="flex p-0.5">
                  <button
                    disabled={
                      !!registerFair ||
                      validateDisable(EnumRegisterCompanyStep.REGISTER_COMPANY)
                    }
                    className="flex flex-col w-1/4 items-center gap-1"
                    title={
                      !!registerFair
                        ? "Finalize o cadastro da feira"
                        : "Informações do cadastro da empresa"
                    }
                    onClick={() =>
                      handleUpdateView(EnumRegisterCompanyStep.REGISTER_COMPANY)
                    }
                  >
                    <CompanyIcon />
                    <p className="hidden md:flex text-sm">Cadastro empresa</p>
                    <hr className="h-1 bg-secondary rounded-md w-full" />
                  </button>
                  <button
                    disabled={
                      !!registerFair ||
                      validateDisable(EnumRegisterCompanyStep.REGISTER_OWNER)
                    }
                    className="flex flex-col w-1/4 items-center gap-1"
                    title={
                      !!registerFair
                        ? "Finalize o cadastro da feira"
                        : "Informações do cadastro do dono da empresa"
                    }
                    onClick={() =>
                      handleUpdateView(EnumRegisterCompanyStep.REGISTER_OWNER)
                    }
                  >
                    <PersonOwnerIcon />
                    <p className="hidden md:flex text-sm">Dono empresa</p>
                    <hr
                      className={`h-1 w-full bg-primary-light rounded-md
                                        ${
                                          stepsTaken.includes(
                                            EnumRegisterCompanyStep.REGISTER_OWNER
                                          ) && "bg-secondary"
                  ...
                  <button
                    disabled={validateDisable(EnumRegisterCompanyStep.FAIRS)}
                    className="flex flex-col w-1/4 items-center gap-1"
                    onClick={() =>
                      handleUpdateView(EnumRegisterCompanyStep.FAIRS)
                    }
                  >
                    <FairIcon />
                    <p className="hidden md:flex text-sm">Seleção feiras</p>
                    <hr
                      className={`h-1 w-full bg-primary-light rounded-md
                                        ${
                                          stepsTaken.includes(
                  ...
                  <button
                    disabled={
                      !!registerFair ||
                      validateDisable(EnumRegisterCompanyStep.SELECT_PLAN)
                    }
                    title={
                      !!registerFair
                        ? "Finalize o cadastro da feira"
                    ...
                    className="flex flex-col w-1/4 items-center gap-1"
                    onClick={() =>
                      handleUpdateView(EnumRegisterCompanyStep.SELECT_PLAN)
                    }
                  >
                    <DollarSymbolIcon />
                    <p className="hidden md:flex text-sm">Seleção do plano</p>
                    <hr
                      className={`h-1 w-full bg-primary-light rounded-md
                                        ${
                                          stepsTaken.includes(
                                            EnumRegisterCompanyStep.SELECT_PLAN
                                          ) && "bg-secondary"
                                        }`}
                    />
                  </button>
              ...
              <>
                ...
                <nav className="flex w-fit gap-8 border-b border-b-primary pb-0.5 mb-10 self-center pr-8 md:self-start md:mb-8">
                  <button
                    ...
                    className={`text-secondary text-xs pl-8 ${
                      isSelectedScreen(EnumCompanyScreen.COMPANY)
                        ? "font-bold"
                  ...
                  <button
                    ...
                    className={`text-secondary text-xs ${
                      isSelectedScreen(EnumCompanyScreen.FAIRS)
                        ? "font-bold"
                        : "hover:text-secondary-light"
                    }`}
                  >
                  ...
                  <button
                    onClick={() =>
                      setScreenProcess(EnumCompanyScreen.BANK_ACCOUNT)
                    }
                    title={"Realize o gerenciamento da conta"}
                    className={`text-secondary text-xs ${
                      isSelectedScreen(EnumCompanyScreen.BANK_ACCOUNT)
                        ? "font-bold"
                        : "hover:text-secondary-light"
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`text-secondary text-xs ${
                      isSelectedScreen(EnumCompanyScreen.POS)
                        ? "font-bold"
                        : "hover:text-secondary-light"
                    }`}
                  >
                ...
                <div className="lg:h-full">
                            ...
                            <div className="flex flex-col gap-6 md:flex-row">
                              <span className="w-full md:w-1/3">
                                <CustomInput
                                  ...
                                  invalid={
                                    company?.companyName
                                      ? !isValidString(company.companyName)
                                      : false
                                  }
                              ...
                              <span className="md:w-1/3">
                                <CustomInput
                                  ...
                                  invalid={
                                    company?.tradingName
                                      ? !isValidString(company.tradingName)
                                      : false
                                  }
                                  onChange={(e) => handleChange(e, setCompany)}
                                />
                              ...
                              <span className="flex items-center gap-1 md:w-[48%]">
                                <span className="w-full">
                                  <CustomInput
                                    ...
                                    onChange={(e) =>
                                      handleCurrencyChange(e, setCompany)
                                    }
                                  />
                                </span>
                                <span
                          ...
                          <div className="flex flex-col md:flex-row gap-8">
                            <div>
                              <p className="text-dark-color">
                                Para facilitar o cadastramento, informe seu
                                CNPJ:
                              </p>
                              ...
                              <span className="w-32 flex mt-6">
                                <Button
                                  text="Consultar"
                                  onClick={handleConsultCompanyData}
                                />
                              </span>
                            </div>
                            ...
                            <div className="flex flex-col gap-6 md:flex-row">
                              ...
                              <span className="w-full md:w-1/3">
                                <CustomInput
                                  ...
                                  invalid={
                                    owner?.name
                                      ? !isValidString(owner.name)
                                      : false
                                  }
                                  inputHeaderText="Seu nome"
                              ...
                              <span className="md:w-[28.7%]">
                                <CustomInput
                                  id="document"
                                  maxLength={14}
                                  value={formatCPFInput(owner?.document ?? "")}
                              ...
                              <span className="w-full flex gap-6">
                                <span className="w-full">
                                  <CustomInput
                                    id="phone"
                                    type="tel"
                                    value={formatPhoneInput(owner?.phone ?? "")}
                                    placeHolder="Insira seu telefone"
                                    inputHeaderText="Telefone"
                                    onChange={(e) => handleChange(e, setOwner)}
                                    maxLength={15}
                                ...
                                <span className="w-full">
                                  <CustomInput
                                    ...
                                    inputHeaderText="Data nascimento"
                                    onChange={(e) => handleChange(e, setOwner)}
                                  />
                                </span>
                              </span>
                              ...
                              <span className="flex items-center gap-1 md:w-[69.5%]">
                                <span className="w-full">
                                  <CustomInput
                                    id="monthlyIncome"
                                    type="numeric"
                                    placeHolder="Insira sua receita mensal"
                                    invalid={
                                      owner?.monthlyIncome
                                        ? !isValidPositiveStrNumber(
                                            owner?.monthlyIncome.toString()
                                          )
                                        : false
                                    }
                                    value={formatCurrency(
                                    ...
                                    inputHeaderText="Receita mensal"
                                    onChange={(e) =>
                                      handleCurrencyChange(e, setOwner)
                                    }
                                  />
                                </span>
                                <span title="Informações necessárias para cadastro na empresa da maquininha(Adquirente).">
                          ...
                          <>
                            <div className="h-fit flex flex-col gap-4 md:flex-row">
                              <div className="flex items-top gap-2">
                                <label
                                  className="relative flex pt-0.5 items-top cursor-pointer"
                                  htmlFor="user-data-input"
                                >
                                  <input
                                    id="user-data-input"
                                    name="color"
                            ...
                            <span className="w-32 flex mt-6">
                              <Button
                                text="Continuar"
                                onClick={handleSelectInfoOwnerFlow}
                              />
                            </span>
                          </>
                        )}
                      ...
                      {registerFair ? (
                        <>
                          <div className="w-fit flex flex-col gap-1 mt-8">
                            ...
                            <p className="text-sm">
                              Será possível utilizar a nova feira em seu
                              cadastro, porém{" "}
                              <strong>
                                só estará disponível para outros usuários após
                                validarmos
                              </strong>{" "}
                              o cadastro.
                            </p>
                          ...
                          <div className="mt-8">
                            <div className="flex flex-col gap-6 w-full lg:w-11/12 xl:w-8/12">
                              <button
                                onClick={() => setRegisterFair(undefined)}
                                className="flex w-fit gap-4"
                              >
                                <ArrowLeftIcon />
                                <p className="text-secondary">Voltar</p>
                              ...
                              <div className="flex flex-col gap-6 md:flex-row">
                                <span className="md:w-1/3">
                                  <CustomInput
                                    id="name"
                                    value={registerFair?.name ?? ""}
                                    inputHeaderText="Nome da feira"
                                    placeHolder="Nome da feira (Ex: Feirinha do centro)"
                                    invalid={
                                      registerFair?.name
                                        ? !isValidString(registerFair.name)
                                        : false
                                    }
                                    onChange={(e) =>
                                      handleChange(e, setRegisterFair)
                                    }
                                  />
                                </span>
                                <span className="w-full md:w-2/3">
                                  <CustomInput
                                    id="description"
                                    value={registerFair?.description ?? ""}
                              ...
                              <div className="flex flex-col gap-6 md:flex-row">
                                <span className="w-full">
                                  ...
                                  <div className="flex flex-wrap">
                                    {Array.from(weekDayMap.keys()).map(
                                      (day) => (
                                        <div
                                          key={day}
                                          id={day}
                                          className="flex mt-4 items-center md:first:-ml-6"
                                        >
                                          <button
                                            onClick={() =>
                                              handleChangeFairWeekDay(day)
                                            }
                                            className={`${
                                              registerFair.daysOfWeek?.includes(
                                                day
                                              ) && "bg-primary"
                                            }
                              ...
                              <div className="flex gap-6">
                                <span className="w-1/2 md:w-1/5">
                                  <CustomInput
                                    id="startAt"
                                    value={formatHHmmInput(
                                      registerFair?.startAt ?? ""
                                    )}
                                    inputHeaderText="Horário início"
                                    maxLength={5}
                                    placeHolder="Horário início (Ex: 14:00)"
                                    invalid={
                                      registerFair.startAt
                                        ? !validFairHour(registerFair.startAt)
                                        : false
                                    }
                                    onChange={(e) =>
                                      handleChange(e, setRegisterFair)
                                ...
                                <span className="w-1/2 md:w-1/5">
                                  <CustomInput
                                    ...
                                    inputHeaderText="Horário fim"
                                    maxLength={5}
                                    placeHolder="Horário fim (Ex: 19:00)"
                                    invalid={
                                      registerFair.endAt
                                        ? !validFairHour(registerFair.endAt)
                                        : false
                                    }
                                    onChange={(e) =>
                                      handleChange(e, setRegisterFair)
                              ...
                              <AddressForm
                                address={registerFair?.address}
                                setObject={setRegisterFair}
                                isParentSetObject
                                showMessage
                              />
                            </div>
                        ...
                        <div className="bg-white p-6 w-full rounded-2xl shadow-lg mt-10">
                          <div className="flex justify-between">
                            ...
                            <span className="flex gap-4 md:gap-6">
                              <span className="flex gap-4 md:gap-6">
                                <CustomSelectOne
                                  handleSelect={handleSelectUF}
                                  optionValue={fairsFilter.state}
                                  options={siglasUF}
                                  inputHeaderText="UF"
                                ...
                                <CustomSelectOne
                                  handleSelect={(city) =>
                                    handleSelectCityFetchFairs(city)
                                  }
                                  inputHeaderText="Cidade"
                                  placeHolder="Selecione UF"
                                  optionValue={fairsFilter.city}
                                  options={fairsFilter.citiesByState}
                                />
                              </span>
                              <span className="hidden md:flex w-36">
                                <Button
                                  text="Cadastrar feira"
                                  onClick={handleStartCreateFair}
                                />
                              </span>
                              <button
                                className="flex w-10 md:hidden h-10 justify-center 
                          ...
                          <div className="w-full border rounded-md mt-6">
                            {fairs?.length != 0 ? (
                              fairs?.map((fair) => (
                                <div
                                  key={fair.id}
                                  className="flex py-4 items-center border-b last:border-b-0"
                                >
                                  <button
                                    onClick={() =>
                                      handleChangeFairCompany(fair)
                                    }
                                    className={`${
                                      company?.fairs?.includes(fair) &&
                                      "bg-primary"
                                    }
                                                                    min-w-5 min-h-5 rounded-md border mx-6 border-gray-300`}
                                  />
                                  <p className="text-xs md:text-sm">
                                    {fair.name} - Rua {fair.address?.street} -
                                    nº{fair.address?.number}
                                  </p>
                                </div>
                              ))
                            ) : (
                              <div className="flex py-4 items-center justify-center">
                                <p>Nenhuma feira encontrada para o filtro.</p>
                              </div>
                            )}
                            {fairs == undefined && (
                              <div className="flex py-4 items-center justify-center">
                  ...
                  {equalsEnum(
                    ...
                    <div className="w-full mt-8 flex flex-col gap-12 items-center justify-center lg:h-full lg:flex-row md:gap-14">
                      ...
                      >
                        ...
                        <ul className="text-left flex flex-col gap-2 mt-4">
                          <li className="flex gap-2">
                            <CheckboxCircleIcon />1 Mês de acesso grátis
                          </li>
                          <li className="flex gap-2">
                            <CheckboxCircleIcon />
                          ...
                          <li className="flex gap-2">
                            ...
                            <p>
                              Após período gratuito você pode migrar para o
                              plano com máquininha ou mensal
                            </p>
                          </li>
                        </ul>
                      </button>
                      <button
                        onClick={() => setSelectedPlan(EnumPlanType.POS_PLAN)}
                        className={`h-fit relative flex flex-col w-full md:w-fit border-2 rounded-md p-4
                                            ${
                                              equalsEnum(
                                                selectedPlan,
                                                EnumPlanType.POS_PLAN
                                              )
                      ...
                      >
                        ...
                        <ul className="mt-4 text-left flex flex-col gap-2 h-fit 2xl:h-36">
                          <li className="flex flex-col">
                            <span className="flex">
                              <span className="flex gap-2">
                                ...
                                <p>0.99% das transações*</p>
                              </span>
                              <span
                                onClick={() => setShowAllTaxes(!showAllTaxes)}
                                className="px-1"
                                title="Descritivo de todos os valores"
                              >
                            ...
                            {showAllTaxes && (
                              <span className="ml-4">
                                <p>
                                  - Percentual referente ao preço do uso do
                                  sistema
                                </p>
                                <p>
                                  - Taxa da maquininha de cartão{" "}
                                  <strong>NÃO INCLUSA</strong>
                                </p>
                              </span>
                            )}
                          </li>
                          ...
                          <li className="flex gap-2">
                            ...
                            <p>
                              Pagamento com{" "}
                              <strong>MAQUININHA INTEGRADA</strong> e{" "}
                              <strong>PIX</strong>
                            </p>
                          </li>
                        </ul>
                      ...
                      <button
                        onClick={() =>
                          setSelectedPlan(EnumPlanType.MONTHLY_PLAN)
                        }
                        className={`h-2/3 md:h-fit flex flex-col w-full md:w-96 border-2 border-second-light rounded-md p-4 shadow-md 
                                                ${
                                                  equalsEnum(
                                                    selectedPlan,
                                                    EnumPlanType.MONTHLY_PLAN
                                                  ) && "border-tertiary"
                                                }`}
                      >
                        ...
                        <ul className="text-left flex flex-col gap-2 mt-4">
                          ...
                          <li className="flex gap-2">
                            <CheckboxCircleIcon />
                            Forma de pagamento <strong>dinheiro</strong> e{" "}
                            <strong>PIX</strong>
                          </li>
                          <li className="flex gap-2">
                            <CheckboxCircleIcon />
              ...
              <div className="flex flex-col h-full">
                <div className="flex flex-col gap-6 w-full lg:w-11/12 xl:w-8/12">
                  <nav className="flex w-fit gap-8 mb-6 md:mb-2">
                    <button
                      onClick={() => setCompanyDataView(true)}
                      className={`text-secondary text-xs ${
                        companyDataView
                          ? "font-bold"
                          : "hover:text-secondary-light"
                    ...
                    >
                      {"Dados ".concat(
                        isIndividualCompany() ? "do autônomo" : "da empresa"
                      )}
                    </button>
                    {!isIndividualCompany() && (
                      <button
                        onClick={() => setCompanyDataView(false)}
                    ...
                    <div className="flex flex-col gap-6">
                      <div className="flex flex-col md:flex-row gap-6">
                        ...
                        <span className="w-full md:w-1/3">
                          <CustomInput
                            ...
                            invalid={
                              company?.owner.name
                                ? !isValidString(company?.owner.name)
                                : false
                            }
                            inputHeaderText="Seu nome"
                            onChange={(e) => handleChange(e, setOwner)}
                        ...
                        <span className="w-full md:w-2/4">
                          <CustomInput
                            ...
                            invalid={
                              company?.owner.email
                                ? !validateEmail(company?.owner.email)
                                : false
                            }
                          />
                        </span>
                        <span className="md:w-[28.7%]">
                          <CustomInput
                            id="document"
                            maxLength={14}
                            value={formatCPFInput(
                              company?.owner.document ?? ""
                            )}
                            disabled
                            inputHeaderText="Documento"
                            onChange={(e) => handleChange(e, setOwner)}
                            invalid={
                              company?.owner.document
                                ? !validateCPF(company?.owner.document)
                                : false
                            }
                          />
                        </span>
                      ...
                      <div className="flex flex-col gap-6 md:flex-row">
                        <span className="w-full flex gap-6">
                          <span className="w-full">
                            <CustomInput
                              ...
                              invalid={
                                company?.owner.phone
                                  ? !validatePhone(company?.owner.phone)
                                  : false
                              }
                            />
                          </span>
                          <span className="w-full">
                            <CustomInput
                              ...
                              invalid={
                                company?.owner.birthdate
                                  ? company?.owner.birthdate?.length != 10
                                  : false
                              }
                              inputHeaderText="Data nascimento"
                              onChange={(e) => handleChange(e, setOwner)}
                        ...
                        <span className="w-full">
                          <CustomInput
                            id="occupation"
                            value={company?.owner.occupation ?? ""}
                            invalid={
                              company?.owner.occupation
                                ? !isValidString(company?.owner.occupation)
                                : false
                            }
                            placeHolder="Informe sua ocupação (Ex: Vendedor, Agricultor)"
                            inputHeaderText="Ocupação"
                        ...
                        <span className="flex items-center gap-1 md:w-[69.5%]">
                          <span className="w-full">
                            <CustomInput
                              ...
                              placeHolder="Insira sua receita mensal"
                              invalid={
                                company?.owner.monthlyIncome
                                  ? !isValidPositiveStrNumber(
                                      company?.owner.monthlyIncome.toString()
                                    )
                                  : false
                              }
                              value={formatCurrency(
                                company?.owner.monthlyIncome ?? 0
                              )}
                              inputHeaderText="Receita mensal"
                              onChange={(e) =>
                                handleCurrencyChange(e, setOwner)
                    ...
                    <div className="w-full border rounded-md mt-6">
                      {company?.fairs?.length != 0 ? (
                        company?.fairs?.map((fair) => (
                          ...
                          >
                            <button
                              onClick={() => handleChangeFairCompany(fair)}
                              className={`${
                                company?.fairs?.includes(fair) && "bg-primary"
                              }
                                                                    min-w-5 min-h-5 rounded-md border mx-6 border-gray-300`}
                            />
                            <p className="text-xs md:text-sm">
                              {fair.name} - Rua {fair.address?.street} - nº
                              {fair.address?.number}
                            </p>
                          </div>
                        ))
                        ...
                        <div className="flex py-4 items-center justify-center">
                          <p>Nenhuma feira cadastrada na empresa.</p>
                        </div>
                      )}
                    </div>
                  </div>
            ...
            {equalsEnum(screenProcess, EnumCompanyScreen.BANK_ACCOUNT) && (
              <>
                    ...
                    {acquirerConfig?.status && (
                      <span className="flex items-center">
                        <p className="font-normal text-dark-color">
                          Status conta:{" "}
                          {getAccountStatusDescription(acquirerConfig?.status)}
                        </p>
                        {equalsEnum(
                          EnumAcquirerConfigStatus.ACTIVE,
                          acquirerConfig?.status
                        ) ? (
                    ...
                    <div className="flex flex-col gap-6 w-full lg:w-11/12 xl:w-8/12">
                      <div className="flex flex-col gap-6 md:flex-row w-full">
                        <span className="w-full md:w-[328px]">
                          <CustomInput
                            id="bankAccount.holderDocument"
                            disabled
                            value={
                              acquirerConfig?.bankAccount?.holderDocument ?? ""
                            }
                            inputHeaderText="Documento"
                            onChange={(e) => handleChange(e, setAcquirerConfig)}
                          />
                        </span>
                        <span className="w-full md:w-[70%]">
                          <DefaultSelectOne
                            handleSelect={defineBank}
                            height="max-h-64"
                        ...
                        <span className="w-full flex gap-6 md:flex-row md:w-4/5">
                          <span className="w-full md:w-44">
                            <CustomInput
                              id="bankAccount.branchNumber"
                              type="numeric"
                              maxLength={4}
                              value={
                                acquirerConfig?.bankAccount?.branchNumber ?? ""
                              }
                              inputHeaderText="Agência"
                              onChange={(e) =>
                                handleChange(e, setAcquirerConfig)
                              }
                            />
                          </span>
                          <span className="w-full">
                            <CustomInput
                              ...
                              type="numeric"
                              placeHolder="Dígito verif. Ag. (Opcional)"
                              maxLength={1}
                              value={
                                acquirerConfig?.bankAccount?.branchCheckDigit ??
                                ""
                              }
                              inputHeaderText="Dígito verif. Agência"
                              onChange={(e) =>
                                handleChange(e, setAcquirerConfig)
                              }
                            />
                      ...
                      <div className="w-full flex flex-col gap-6 md:flex-row">
                        <span className="w-full flex gap-6 md:flex-row">
                          ...
                          <span className="w-full lg:w-1/2">
                            <CustomInput
                              ...
                              inputHeaderText="Dígito verif. num. conta"
                              onChange={(e) =>
                                handleChange(e, setAcquirerConfig)
                              }
                            />
                        ...
                        <span className="w-full lg:w-1/2">
                          <DefaultSelectOne
                            handleSelect={defineAccountType}
                            optionValue={getAccountType(
                              acquirerConfig?.bankAccount?.type
                            )}
                            options={["Conta Corrente", "Conta Salário"]}
                            inputHeaderText="Tipo conta"
                          />
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-6 w-full xl:w-8/12">
                      <div className="flex flex-col gap-6 md:flex-row lg:w-1/2">
                        <span className="w-full md:w-1/2">
                          <DefaultSelectOne
                            handleSelect={defineTransferInterval}
                            optionValue={getTransferIntervalDescription(
                              acquirerConfig?.transferConfig
                                ?.transferInterval as EnumTransferIntervalType
                            )}
                            options={["Mensal", "Semanal", "Diário"]}
                            inputHeaderText="Intervalo de transferência"
                          />
                        </span>
                        {equalsEnum(
                          acquirerConfig?.transferConfig?.transferInterval,
                          ...
                          <span className="md:w-1/2">
                            <DefaultSelectOne
                              handleSelect={defineWeekDay}
                              optionValue={getWeekDayDescription(
                                acquirerConfig?.transferConfig
                                  ?.transferDay as EnumWeekDaysType
                              )}
                              options={[
                                "Segunda-feira",
                                "Terça-feira",
                        ...
                        {equalsEnum(
                          acquirerConfig?.transferConfig?.transferInterval,
                          EnumTransferIntervalType.MONTHLY
                        ) && (
                          <span className="md:w-1/2">
                            <CustomInput
                              id="transferConfig.transferDay"
                              type="numeric"
                              placeHolder="Insira o dia do mês"
                              invalid={
                                acquirerConfig?.transferConfig?.transferDay
                                  ? !(
                                      isValidPositiveStrNumber(
                                        acquirerConfig?.transferConfig?.transferDay.toString()
                                      ) &&
                                      (acquirerConfig?.transferConfig
                                        ?.transferDay as number) <= 31
                                    )
                                  : false
                              }
                              maxLength={2}
                      ...
                      <p className="-mb-4 text-xs md:text-sm">
                        Dia padrão de realização do TED do recebimento é toda
                        segunda-feira, caso não seja feriado. Valor taxa TED é
                        de <span className="font-semibold">R$ 3,50</span>.
                      </p>
                      <p className="text-xs md:text-sm">
                        O valor do TED será o total de vendas do período,
                        descontando as taxas e o valor de antecipação, caso
                        esteja ativo.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="h-fit flex mt-8 gap-10">
                    <div className="flex flex-col gap-6 w-full xl:w-8/12">
                      <div className="flex flex-col gap-6 md:flex-row lg:w-1/2">
                        <span className="w-full md:w-1/2">
                          <DefaultSelectOne
                            handleSelect={enableAnticipation}
                            optionValue={
                              acquirerConfig?.anticipationConfig?.enabled
                                ? "ATIVADA"
                                : "DESATIVADA"
                            }
                            options={["ATIVADA", "DESATIVADA"]}
                            inputHeaderText="Antecipação automática"
                          />
                        </span>
                        <span className="w-full md:w-1/2">
                          {acquirerConfig?.anticipationConfig?.enabled && (
                            <CustomInput
                              ...
                              value={"100%"}
                              disabled
                              inputHeaderText="Percentual de antecipação"
                              onChange={() => false}
                            />
                          )}
                        </span>
                      ...
                      {acquirerConfig?.anticipationConfig?.enabled && (
                        <p className="text-xs md:text-sm">
                          Caso ativo, o valor da antecipação de recebíveis por
                          parcela é de <b>1.4%</b>.
                        </p>
                      )}
                    </div>
                  </div>
                ...
                <div className="w-full h-10 flex mt-10 md:mt-auto justify-end">
                  <span className="w-full md:w-40">
                    <Button
                      disabled={!isValidBankAccountData()}
                      text={
                        company?.hasAcquirerConfig ? "Atualizar" : "Cadastrar"
                      }
                ...
                <div className="bg-white p-6 w-full rounded-2xl shadow-lg">
                  <div className="flex justify-between">
                    ...
                    <span className="flex gap-4 md:gap-6 w-28">
                      <Button
                        text="Solicitar"
                        className="border border-tertiary-light bg-white text-tertiary"
                    ...
                    <div className="flex py-4 justify-between items-center border-b last:border-b-0">
                      <span className="flex gap-4">
                        ...
                        <p className="text-xs md:text-sm">
                          ...
                          <b className="font-semibold mr-1 ml-4">Descrição:</b>
                          Maquininha utilizada no dia-a-dia
                          <b className="font-semibold mr-1 ml-4">Status:</b>
                  ...
                  <div className="w-full border rounded-md mt-6 px-6 md:mb-4">
                    <div className="flex py-4 justify-between items-center border-b last:border-b-0">
                      <span className="flex gap-4">
                        ...
                        <p className="text-xs md:text-sm">
                          <b className="font-semibold mr-1 ml-4">
                            21/02/2025 08:10:20
                          </b>{" "}
                          - Maquininha recebida, pendente de ativação
                        </p>
                      </span>
                    </div>
                    <div className="flex py-4 justify-between items-center border-b last:border-b-0">
                    ...
                    <div className="flex py-4 justify-between items-center border-b last:border-b-0">
                      <span className="flex gap-4">
                        ...
                        <p className="text-xs md:text-sm">
                          ...
                          - Realizada as configurações para a empresa do cliente
                        </p>
                      </span>
                    </div>
                    <div className="flex py-4 justify-between items-center border-b last:border-b-0">
                      <span className="flex gap-4">
                        ...
                        <p className="text-xs md:text-sm">
                          ...
                          - Recebida maquininha do parceiro pelo time
                          feira-fácil
                        </p>
                      </span>
                    </div>
